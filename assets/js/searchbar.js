...
document.getElementById("search-bar").addEventListener("input", () => {
    ...
    const suggestionsDiv = document.getElementById("suggestions"); // A div to display suggestions
    resultsDiv.innerHTML = ""; // Clear previous results

    if (query) {
        // Find vehicles with model names that contain the query string (case-insensitive)
        const matchingVehicles = Object.keys(vehicleDetails).filter(vehicle => vehicle.toLowerCase().includes(query));

        // Show suggestions if there are any matches
        if (matchingVehicles.length > 0) {
            suggestionsDiv.innerHTML = matchingVehicles.map(vehicle => `<p class="suggestion-item">${vehicle}</p>`).join('');
            suggestionsDiv.style.display = "block"; // Show the suggestions

            // Adjust the width of the suggestions box to match the search bar
            const searchBar = document.getElementById("search-bar");
            const searchBarRect = searchBar.getBoundingClientRect();
            suggestionsDiv.style.width = `${searchBarRect.width}px`; // Match the width of the search bar
            suggestionsDiv.style.left = `${searchBarRect.left}px`; // Align the left position with the search bar
        } else {
            suggestionsDiv.innerHTML = "<p>No suggestions found</p>";
            suggestionsDiv.style.display = "block"; // Show a message when no suggestions are found
        }
    } else {
        suggestionsDiv.style.display = "none"; // Hide suggestions if the input is empty
    }
});

// When a suggestion is clicked, populate the search bar with the selected suggestion
document.getElementById("suggestions").addEventListener("click", (event) => {
    if (event.target && event.target.classList.contains("suggestion-item")) {
        ...
        document.getElementById("search-bar").value = selectedModel;
        document.getElementById("suggestions").style.display = "none"; // Hide suggestions after selection
        searchVehicle(selectedModel); // Trigger search
    }
});
...
// Separate function to trigger the search after the user selects a suggestion
function searchVehicle(query) {
    const resultsDiv = document.getElementById("search-results");
    resultsDiv.innerHTML = ""; // Clear previous results

    if (query) {
        // Find the vehicle details by exact match
        const matchingVehicle = Object.keys(vehicleDetails).find(vehicle => vehicle.toLowerCase() === query.toLowerCase());

        if (matchingVehicle) {
            const vehicle = vehicleDetails[matchingVehicle];
                ...
                <table class="vehicle-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>${vehicle.engineSpecifications}</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>${vehicle.priceRange}</td>
                        </tr>
                        <tr>
                            <td><strong>Mileage</strong></td>
                            <td>${vehicle.mileageRange}</td>
                        </tr>
                        <tr>
                            <td><strong>Safety Features</strong></td>
                            <td>${vehicle.safetyFeatures}</td>
                        </tr>
                        <tr>
                            <td><strong>Color Options</strong></td>
                            <td>${vehicle.colorOptions}</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>${vehicle.performanceFeatures}</td>
                        </tr>
                        <tr>
                            <td><strong>Interior</strong></td>
                            <td>${vehicle.interiorFeatures}</td>
                        </tr>
                        <tr>
                            <td><strong>Exterior</strong></td>
                            <td>${vehicle.exteriorFeatures}</td>
                        </tr>
                        <tr>
                            <td><strong>Warranty</strong></td>
                            <td>${vehicle.warrantyAfterSalesService}</td>
                        </tr>
                        <tr>
                            <td><strong>Release Year</strong></td>
                            <td>${vehicle.releaseDateYear}</td>
                        </tr>
                        <tr>
                            <td><strong>Image</strong></td>
                            <td><img src="${vehicle.imageUrl}" alt="${matchingVehicle}" class="result-image" style="width: 150px; height: auto;" /></td>
                        </tr>
                    </tbody>
                </table>
            `;
        } else {
            resultsDiv.textContent = "No vehicles matched your search.";
        }
    } else {
        resultsDiv.textContent = "Kindly provide a vehicle name to begin your search.";
    }
}
